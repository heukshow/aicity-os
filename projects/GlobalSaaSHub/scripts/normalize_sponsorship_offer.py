...
def main() -> None:
    # The verified-offer hub is generated by the preceding revenue scripts. Extend
    # it here with Murf's already-approved customer route before Vite copies public/
    # into dist. A normal idempotence exit is safe; any evidence/safety failure stops
    # the production build instead of publishing a guessed affiliate destination.
    try:
        runpy.run_path(str(ROOT / "scripts" / "surface_murf_verified_offer.py"), run_name="__main__")
