"""
tests/test_affiliate_url_verifier.py  (v3 - opener.open mock)
=============================================================
Uses _build_opener mock instead of urlopen mock (v3 API).
"""
...
<footer><a href="/privacy">Privacy</a> | <a href="/affiliate">Affiliate</a></footer></body></html>"""

HTML_PARTNERS_WITH_EVIDENCE = b"""<html><head><title>Partner Program</title></head>
<body><h1>Join our Affiliate Program</h1>
<p>Payout terms: monthly. Commission rate: 20%. Affiliate dashboard available.</p></body></html>"""

HTML_PARTNERS_NO_EVIDENCE = b"""<html><head><title>Partners</title></head>
<body><h1>Business Partners</h1><p>Contact us to become a reseller.</p></body></html>"""

HTML_COMMISSION_PATTERN = b"""<html><head><title>Referral</title></head>
<body><h1>Referral Program</h1>
<p>Our referral tracking system ensures you get credit. Cookie duration: 60 days.</p></body></html>"""
...
def _opener_mock(resp):
    """Return a patched _build_opener that yields the given response."""
...
def test_strong_evidence_accepted():
    resp = _make_mock_response(HTML_STRONG, url="https://example.com/affiliate-program")
...
def test_partners_path_with_strong_evidence_accepted():
    resp = _make_mock_response(HTML_PARTNERS_WITH_EVIDENCE, url="https://example.com/partners")
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
...
def test_partners_path_no_evidence_rejected():
    resp = _make_mock_response(HTML_PARTNERS_NO_EVIDENCE, url="https://example.com/partners")
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
        r = verify_affiliate_url("https://example.com/partners")
...
def test_footer_only_affiliate_word_rejected():
    resp = _make_mock_response(HTML_FOOTER_ONLY, url="https://example.com/home")
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
...
def test_commission_pattern_accepted():
    resp = _make_mock_response(HTML_COMMISSION_PATTERN, url="https://example.com/referral")
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
...
def test_redirect_final_url_stored():
    ...
    resp = _make_mock_response(HTML_STRONG, url=final)
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
...
def test_safe_affiliate_result_has_all_fields():
    resp = _make_mock_response(HTML_STRONG, url="https://example.com/affiliate")
    with patch("affiliate_url_verifier._build_opener", return_value=_opener_mock(resp)):
...
if __name__ == "__main__":
    tests = [
        test_strong_evidence_accepted, test_ssl_error_rejected,
        test_partners_path_with_strong_evidence_accepted, test_partners_path_no_evidence_rejected,
        test_footer_only_affiliate_word_rejected, test_commission_pattern_accepted,
        test_redirect_final_url_stored, test_network_error_returns_full_meta,
        test_safe_affiliate_result_has_all_fields, test_normalized_tool_contains_all_affiliate_fields,
    ]
    print("=" * 60)
    print("affiliate_url_verifier v3 - Mock-Based Contract Tests")
    print("=" * 60)
    passed = failed = 0
    for t in tests:
        try:
