...
# - Dorik's first-party partner terms checked 2026-09-13 state 40% on the first
#   purchase, 10% recurring on renewals and a 90-day attribution cookie.
...
if MARKER in html:
    ...
    if any(token in html for token in BLOCKED_PUBLIC_TOKENS):
        raise SystemExit("Dorik partner/admin URL leaked into the public buyer hub")
...
card = f'''      {MARKER}\n      <article class="rounded-3xl border border-cyan-400/25 bg-[#11131a] p-7 space-y-5">\n        <div class="flex items-start justify-between gap-4"><div><div class="text-xs font-black uppercase tracking-wider text-cyan-300">AI website builder</div><h2 class="mt-1 text-3xl font-black text-white">Dorik</h2></div><span class="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-200">7-day free trial</span></div>\n        <p class="text-sm leading-7 text-slate-300">Dorik's live pricing page currently offers a <strong class="text-white">7-day free trial</strong>. Current month-to-month prices are <strong class="text-white">Personal $29/month</strong>, <strong class="text-white">Business $59/month</strong>, and <strong class="text-white">Agency $99/month</strong>. Annual, two-year and lifetime options are also published, so verify the live pricing page before purchasing.</p>\n        <p class="text-sm leading-7 text-slate-300">Dorik's first-party partner terms currently state <strong class="text-white">40% commission on the first purchase</strong>, <strong class="text-white">10% recurring on renewals</strong>, and a <strong class="text-white">90-day cookie</strong>. These are program terms, not COSHUMA earnings.</p>\n        <p class="text-sm leading-7 text-slate-300">COSHUMA uses only the exact customer-facing Dorik referral URL already recorded as <strong class="text-white">approved_tracking</strong>. Partner dashboard/success-hub URLs and guessed pricing deep links are excluded from buyer CTAs.</p>\n        <div class="grid gap-3 sm:grid-cols-2">\n          <a data-cta="affiliate" data-tool-id="dorik" data-cta-source="verified-deals-dorik-trial" data-cta-page="verified-software-free-trials-deals" href="{TRACKING_URL}" target="_blank" rel="sponsored nofollow noopener noreferrer" class="rounded-xl bg-cyan-600 px-5 py-3.5 text-center font-black text-white hover:bg-cyan-500">Try Dorik via verified partner link →</a>\n          <a data-cta="official" href="{OFFICIAL_PRICING}" target="_blank" rel="noopener noreferrer" class="rounded-xl border border-white/15 px-5 py-3.5 text-center font-bold text-slate-200 hover:bg-white/[0.05]">Verify live pricing →</a>\n        </div>\n        <div class="flex flex-wrap gap-x-5 gap-y-2 text-xs font-bold">\n          <a href="{GUIDE_URL}" class="text-violet-300 hover:text-violet-200">Read Dorik buyer guide →</a>\n          <a href="{OFFICIAL_AFFILIATE_TERMS}" target="_blank" rel="noopener noreferrer" class="text-violet-300 hover:text-violet-200">Verify partner terms →</a>\n        </div>\n        <p class="text-xs leading-5 text-slate-500">Affiliate disclosure: COSHUMA may earn commission on eligible purchases made through the verified Dorik partner link. Publishing, opening or validating the link does not prove a click, signup, paid customer, commission, payout or revenue.</p>\n      </article>\n'''
...
if any(token in html for token in BLOCKED_PUBLIC_TOKENS):
    raise SystemExit("Dorik partner/admin URL leaked after patch")

PAGE.write_text(html, encoding="utf-8")
print("Surfaced verified Dorik buyer route")
