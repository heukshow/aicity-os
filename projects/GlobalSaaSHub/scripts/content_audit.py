...
ALLOWED_CROSS_REFERENCES={"seamless","helpdesk","chatbot","shopify"}
...
def audit(online=False,workers=12):
 ...
 for t in tools:
  tid=t.get("id")
  if not online and t.get("official_verification_status") != "verified": issues.append(finding(tid,"UNVERIFIED","recorded_official_evidence_missing",t.get("official_url")))
  ...
  for oid,oname in names:
   if oid!=tid and oname not in ALLOWED_CROSS_REFERENCES and len(oname)>=5 and re.search(rf"\b{re.escape(oname)}\b",desc): issues.append(finding(tid,"UNVERIFIED","possible_cross_tool_copy",{"other_tool":oid,"name":oname}))
 ...
 for t in tools:
  ...
  status=min(actionable,key=lambda s:rank[s]) if actionable else ("FIXED" if t["id"] in FIXED_IDS else ("UNVERIFIED" if any(x["classification"]=="UNVERIFIED" for x in tis) or (not online and t.get("official_verification_status")!="verified") else "VERIFIED"))
  per.append({"id":t["id"],"name":t["name"],"classification":status,"official_url":t.get("official_url"),"pricing_source_url":t.get("pricing_source_url"),"rating":t.get("rating"),"official_verification_status":t.get("official_verification_status"),"official_verified_at":t.get("official_verified_at"),"official_evidence_url":t.get("official_evidence_url"),"issues":tis,"live_evidence":live.get(t["id"],{})})
