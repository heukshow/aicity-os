...
def category_page(slug,cfg):
    ...
    body=f'''<nav class="crumb"><a href="/">Home</a> / Category / {e(slug.replace('-',' ').title())}</nav><div class="badge">Buyer guide · updated {TODAY}</div><h1>{e(cfg['title'])}</h1><p class="lead">{e(cfg['intro'])}</p><section class="decision">{''.join(f'<div><strong>{i+1}</strong><div>{e(x)}</div></div>' for i,x in enumerate(cfg['tips']))}</section><h2>Tools to compare first</h2><p class="lead">This shortlist uses COSHUMA's product records to surface tools worth comparing for the use case above. Open each buyer guide to compare pricing, features and practical trade-offs.</p><section class="grid">{''.join(card(t) for t in items[:8])}</section><h2>Questions buyers ask</h2>{''.join(f'<div class="faq"><h3>{e(q)}</h3><p>{e(a)}</p></div>' for q,a in cfg['faq'])}<section class="method"><strong>How this page is built:</strong> COSHUMA uses recorded product descriptions, pricing verification dates and official-source URLs from its dataset. Missing dates are shown as missing rather than guessed. <a href="/methodology.html">Read the full methodology →</a></section>'''
