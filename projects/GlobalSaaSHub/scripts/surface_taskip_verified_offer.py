...
# - Taskip first-party pricing was rechecked 2026-09-13.
...
# Internal affiliate/account/revenue mechanics must never be emitted into buyer copy.
if MARKER in html:
    ...
    if any(token in html for token in BLOCKED_PUBLIC_TOKENS):
        raise SystemExit("Taskip internal affiliate/verification copy leaked into the public buyer hub")
    print("Taskip offer already surfaced")
    raise SystemExit(0)
...
card = f'''      {MARKER}\n      <article class="rounded-3xl border border-cyan-400/25 bg-[#11131a] p-7 space-y-5">\n        <div class="flex items-start justify-between gap-4"><div><div class="text-xs font-black uppercase tracking-wider text-cyan-300">AI-first client portal for agencies</div><h2 class="mt-1 text-3xl font-black text-white">Taskip</h2></div><span class="rounded-full bg-emerald-400/10 px-3 py-1 text-xs font-bold text-emerald-200">7-day premium trial · no card to sign up</span></div>\n        <p class="text-sm leading-6 text-slate-300">Taskip's current first-party pricing page says its premium trial provides <strong class="text-white">7 days of full access</strong>, and its signup FAQ says <strong class="text-white">no credit card is needed to start</strong>. The current monthly pricing view starts the Freelancer plan at <strong class="text-white">$12/month</strong>.</p>\n        <p class="text-sm leading-6 text-slate-300">Taskip combines a client portal with CRM, projects, invoicing, e-signatures, support and automation for agencies.</p>\n        <div class="grid gap-3 sm:grid-cols-2"><a data-cta="affiliate" data-tool-id="taskip" data-cta-source="verified-deals-taskip-trial" data-cta-page="verified-software-free-trials-deals" href="{TRACKING_URL}" target="_blank" rel="sponsored nofollow noopener noreferrer" class="rounded-xl bg-cyan-600 px-5 py-3.5 text-center text-sm font-black text-white hover:bg-cyan-500">Explore Taskip →</a><a href="{GUIDE_URL}" class="rounded-xl border border-white/10 px-5 py-3.5 text-center text-sm font-bold text-slate-200 hover:bg-white/5">Review Taskip</a></div>\n        <a href="{OFFICIAL_PRICING}" target="_blank" rel="noopener noreferrer" class="inline-block text-xs font-bold text-cyan-200 hover:text-white">Check current pricing & trial →</a>\n      </article>\n'''
...
print("Surfaced customer-facing Taskip trial route on buyer hub")
