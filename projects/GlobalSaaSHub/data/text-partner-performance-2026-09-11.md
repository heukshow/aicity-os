...
- Vendor sender for performance report: Pawel from LiveChat / Text Partner Program `<partners@livechat.com>`
- Gmail thread id: `1a08c0544c56b5ac`
...
Text Support representative Jacob (AI Supervisor @ Text) replied to `support@coshuma.com` in the same thread on 2026-09-12 04:28:41 UTC (Gmail message id `1a093dff3a24f76e`). His operative answer was:

> Currently, all offered Text related Campaigns are those that are visible in the Partner Program panel.

Evidence interpretation:

- Text's available affiliate campaign inventory is the campaign set shown in the Partner Program panel.
- The vendor did **not** provide a separate trial/signup/pricing deep tracking URL in response to COSHUMA's explicit request.
...
This change preserves the already verified attribution route while reducing a conversion/trust mismatch that could cause visitors to expect a dedicated trial landing page the vendor did not provide.
...
- Future campaign changes must come from the authenticated Partner Program panel or another direct vendor confirmation; a generic homepage, dashboard, onboarding URL, or hand-built parameterized URL is not sufficient evidence.
