...
function payerName(order) {
  return [order?.payer?.name?.given_name, order?.payer?.name?.surname].filter(Boolean).join(' ') || null;
}

function validationForAssets(asset) {
  const required = ['companyName', 'productName', 'contactEmail', 'destinationUrl', 'logoUrl', 'headline', 'description', 'ctaText'];
...
async function captureOrder(request, env, repo) {
  ...
  const withPayer = await repo.recordPayer(orderId, payerName(verified), verified?.payer?.email_address || null, now);
  const campaign = await repo.ensureCampaign(withPayer, now);
  ...
  return json({
    ...
    intakeUrl: `${workerBase}/advertiser/intake?token=${campaign.intake_token}`,
    reportUrl: `${workerBase}/advertiser/report?token=${campaign.report_token}`,
...
async function webhook(request, env, repo) {
  ...
  const local = providerOrderId ? await repo.getByProviderOrderId(providerOrderId) : null;
  validatePaidWebhook(event, local?.product_id);
  ...
  try {
    ...
    if (target && providerOrderId) {
      const updated = await repo.transition(providerOrderId, target, now);
      if (target === 'paid') await repo.ensureCampaign(updated, now);
    }
...
async function submitAssets(request, env, repo) {
  ...
  const validation = validationForAssets(asset);
  const updated = await repo.saveAssets(campaign.id, asset, validation.status, validation.notes, new Date().toISOString());
...
async function advertiserReport(request, env, repo) {
  ...
  const campaign = await repo.getCampaignByReportToken(url.searchParams.get('token') || '');
  if (!campaign) return json({ error: 'Campaign not found' }, 404, corsHeaders(request, env));
...
async function campaignEvent(request, env, repo) {
  const body = await request.json().catch(() => ({}));
  if (!['sponsored_impression', 'sponsored_click'].includes(body.eventType)) return json({ error: 'Invalid event type' }, 400, corsHeaders(request, env));
  const campaign = await repo.getCampaignById(body.campaignId || '');
  if (!campaign || campaign.status !== 'published') return json({ error: 'Active campaign not found' }, 404, corsHeaders(request, env));
  await repo.recordEvent(campaign.id, body.eventType, body.page, campaign.placement, body.destinationUrl, new Date().toISOString());
...
export default {
  async fetch(request, env) {
    ...
    if (fixedAdminRoute) return handleAdminRequest(request, { ...env, ADMIN_PATH: PUBLIC_ADMIN_PATH });
