...
function render(data, money) {
  ...
  <article class="card"><div class="section-head"><div><h2>검색 유입 분석</h2><div class="sub">검색 콘솔 상위 검색어</div></div><span class="badge ok">연결됨</span></div><div class="list">${data.searchConsole.topQueries.map(item=>`<div class="row"><b>${escapeHtml(item.query)}</b><span>노출 ${item.impressions} · 클릭 ${item.clicks}</span></div>`).join('')}</div><h3>방문자 분석</h3><div class="notice">구글 애널리틱스 수집을 연결했습니다. 새 방문 데이터가 쌓인 뒤 국가·유입 경로·인기 페이지가 표시됩니다.</div></article>
