...
function render(data, money, partnerStack) {
  ...
  const funnel = [
    ...
    ['전환 및 수익', partnerStack.connected ? `${partnerStack.rewardCount}건 · $${partnerStack.total.toFixed(2)}` : (money.connected ? `${money.paidCount}건` : '연결 안 됨'), 15],
  ...
  <article class="card"><div class="section-head"><div><h2>수익 및 지급</h2><div class="sub">외부 API와 결제 저장소에서 확인된 데이터만 표시</div></div>${partnerStack.connected?'<span class="badge ok">PartnerStack API 연결됨</span>':empty()}</div><div class="quality"><div><span>제휴 발생 수익</span><strong>${partnerStack.connected?`$${partnerStack.total.toFixed(2)}`:'—'}</strong></div><div><span>제휴 지급 대기</span><strong>${partnerStack.connected?`$${partnerStack.pending.toFixed(2)}`:'—'}</strong></div><div><span>제휴 지급 완료</span><strong>${partnerStack.connected?`$${partnerStack.paid.toFixed(2)}`:'—'}</strong></div><div><span>제휴 수익 건수</span><strong>${partnerStack.connected?partnerStack.rewardCount:'—'}</strong></div></div><h3>PartnerStack 프로그램</h3><div class="list"><div class="row"><b>승인 / 대기 / 거절</b><span>${partnerStack.connected?`${partnerStack.statusCounts.approved} / ${partnerStack.statusCounts.pending} / ${partnerStack.statusCounts.declined}`:'확인 필요'}</span></div><div class="row"><b>API 최근 확인</b><span>${partnerStack.connected?escapeHtml(partnerStack.checkedAt):'연결 오류'}</span></div><div class="row"><b>COSHUMA 자체 결제</b><span>${money.connected?`확정 $${money.actual.toFixed(2)} · 미완료 ${Math.round(money.pending / 49)}건`:'연결 오류'}</span></div></div><h3>연결 상태</h3><div class="list"><div class="row"><b>구글 애널리틱스 4</b>${empty(data.connections.ga4)}</div><div class="row"><b>구글 검색 콘솔</b>${empty(data.connections.searchConsole)}</div><div class="row"><b>PartnerStack 제휴 수익</b><span class="badge ${partnerStack.connected?'ok':'bad'}">${partnerStack.connected?'자동 연결됨':'연결 오류'}</span></div></div></article></section>
...
export async function handleAdminRequest(request, env) {
  ...
  let partnerStack = { connected: false, reason: '연결 오류', total: 0, pending: 0, paid: 0, rewardCount: 0, statusCounts: { approved: 0, pending: 0, declined: 0 } };
  try {
    partnerStack = await fetchPartnerStackMetrics(env);
  } catch (error) {
    console.error('PartnerStack API request failed', error?.message);
  }
