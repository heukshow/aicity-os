...
function render(data, money) {
  ...
  <section class="grid metrics">
    ${metricCard('오늘 방문자','연결 안 됨','구글 애널리틱스 연결 필요')}${metricCard('최근 7일 방문자','연결 안 됨','구글 애널리틱스 연결 필요')}${metricCard('최근 30일 방문자','연결 안 됨','구글 애널리틱스 연결 필요')}
    ${metricCard('구글 검색 실적','연결 안 됨','검색 콘솔 인증 필요')}${metricCard('제휴 버튼 클릭','연결 안 됨','분석 이벤트 연결 필요','accent')}${metricCard('검증된 제휴 링크',data.counts.verifiedAffiliates,`전체 ${data.counts.totalTools}개 도구 중`,'accent')}
  </section>
  ...
  <article class="card"><div class="section-head"><div><h2>수익 및 지급</h2><div class="sub">결제 저장소에서 확인된 데이터만 표시</div></div>${money.connected?'<span class="badge ok">결제 저장소 연결됨</span>':empty()}</div><div class="quality"><div><span>확정 수익</span><strong>${money.connected?`$${money.actual.toFixed(2)}`:'—'}</strong></div><div><span>결제 대기</span><strong>${money.connected?`$${money.pending.toFixed(2)}`:'—'}</strong></div><div><span>지급 완료</span><strong>확인 필요</strong></div><div><span>전환 건수</span><strong>${money.connected?money.paidCount:'—'}</strong></div></div><h3>연결 상태</h3><div class="list"><div class="row"><b>구글 애널리틱스 4</b>${empty(data.connections.ga4)}</div><div class="row"><b>구글 검색 콘솔</b>${empty(data.connections.searchConsole)}</div><div class="row"><b>제휴 프로그램 수익</b>${empty('연결 안 됨')}</div></div></article></section>
  <section class="grid three"><article class="card"><div class="section-head"><div><h2>검색 최적화 품질 상태</h2><div class="sub">저장소 기반 실제 검사 결과</div></div><span class="badge ${data.seo.testsPassed?'ok':'bad'}">${data.seo.testsPassed?'통과':'확인 필요'}</span></div><div class="quality"><div><span>사이트맵 주소</span><strong>${data.counts.sitemapUrls}</strong></div><div><span>도구 페이지</span><strong>${data.counts.toolPages}</strong></div><div><span>비교 페이지</span><strong>${data.counts.comparePages}</strong></div><div><span>한글 혼입</span><strong>${data.seo.koreanLeakCount}</strong></div></div><h3>검사 항목</h3><div class="list">${['404 및 깨진 내부 링크','대표 주소 정합성','검색 로봇 설정','공개 페이지 한글 혼입'].map(label=>`<div class="row"><b>${label}</b><span class="badge ${data.seo.testsPassed?'ok':'muted'}">${data.seo.testsPassed?'통과':'확인 필요'}</span></div>`).join('')}</div></article>
  <article class="card"><div class="section-head"><div><h2>유입 분석</h2><div class="sub">국가 · 유입 경로 · 인기 페이지</div></div>${empty()}</div><div class="notice">구글 애널리틱스 측정 ID와 데이터 조회 인증 정보가 아직 확인되지 않았습니다. 연결 전까지 방문자·국가·유입 경로·인기 페이지는 표시하지 않습니다.</div><h3>최근 검색 최적화 변경</h3><div class="list">${data.recentSeoChanges.map(item=>`<div class="row"><b>${escapeHtml(item.title)}</b><span>${escapeHtml(item.date)}</span></div>`).join('')}</div></article>
